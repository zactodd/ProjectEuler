...
def count_all_unconcealed(prime):
    return [count_unconcealed(prime, e) if math.gcd(e, prime - 1) == 1 else 10 ** 20 for e in range(prime - 1)]


def count_unconcealed(modulus, e):
    return sum(pow(m, e, modulus) == m for m in range(modulus))


def answer(p=1009, q=3643):
    uncon_p, uncon_q = count_all_unconcealed(p), count_all_unconcealed(q)
    min_p, min_q = min(uncon_p), min(uncon_q)
    return sum(e for e in range((p - 1) * (q - 1)) if uncon_p[e % (p - 1)] == min_p and uncon_q[e % (q - 1)] == min_q)
